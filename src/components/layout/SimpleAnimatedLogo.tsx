...
// Define path data for each letter
const letterPaths = [
  // U
...
const SimpleAnimatedLogo = () => {
  ...
  useEffect(() => {
    ...
    // Clear previous paths if any exist
    const existingPaths = svgRef.current.querySelectorAll("path");
    existingPaths.forEach((path) => path.remove());

    // Create individual path elements for each letter
    letterPaths.forEach((pathData, index) => {
      ...
      pathRefs.current[index] = pathElement;
    });
  }, []);

  // Animate paths dropping
  useEffect(() => {
    if (pathRefs.current.length === 0) return;

    // Setup Intersection Observer to trigger animation when SVG is visible
  ...
  return (
    <svg
      ...
      viewBox="128.009 -64.155 140.11 27.104"
      width="100%"
      className="mt-4 w-full"
    >
      {/* Path elements will be added programmatically via useEffect */}
    </svg>
  );
};
